...
def backfillDatabase(buckets: List[dict], account_id: str) -> dict:
    try:
        ...
        existing_db_objects = {(obj.bucket_id, obj.key): obj.id for obj in session.query(S3BUCKETOBJECTS).join(S3BUCKETS, S3BUCKETS.id == S3BUCKETOBJECTS.bucket_id).filter(S3BUCKETS.account_id == account_id).all()}
        ...
        for bucket in buckets:
            try:
                ...
                if bucket_id:
                    logger.info(f'Bucket {bucket["bucket"]} already exists in the database. Checking for updates...')
                ...
                for obj in bucket['objects']:
                    object_key = existing_db_objects.get((bucket_id, obj['key']))
                    if object_key:
                        logger.info(f'{obj["key"]} exists in the database. Checking for updates...')
                        if objectNeedsUpdate(session, object_key, obj):
                            modifyObject(session, object_key, obj)
                        logger.info("Ok.")
                ...
                current_keys |= {(bucket_id, obj['key']) for obj in bucket['objects']}
...
def objectNeedsUpdate(session: sessionmaker, object_id: int, object: Dict) -> bool:
    existing_object = session.query(S3BUCKETOBJECTS).filter(S3BUCKETOBJECTS.id == object_id).one()
    return (
        existing_object.sizeBytes != object['sizeBytes'] or
        existing_object.sizeKb != object['sizeKb'] or
        existing_object.sizeMb != object['sizeMb'] or
        existing_object.sizeGb != object['sizeGb'] or
        existing_object.costPerMonth != object['costPerMonth']
    )
